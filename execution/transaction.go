...
func ReadTransactions(path string, accounts *AccountManager) ([]*types.Transaction, []common.Address) {
	...
	fileScanner := bufio.NewScanner(txJSONContent) // read line by line
	for fileScanner.Scan() {
		t := MyTransaction{}
		err := json.Unmarshal([]byte(fileScanner.Text()), &t) // Deserialization
		if err != nil {
