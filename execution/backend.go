...
func NewBackend(projPath string) (*Backend, []*FuzzLogger) {
	...
	// initialize the account in backend, not in evm
	backend.InitAccountManager()
	// deploy contracts
	loggers := backend.DeployContracts() // return Fuzzlogger
...
func (backend *Backend) DeployContracts() []*FuzzLogger {
	...
	deployedBytecodes := backend.readDeployedBytecodes() // read bytecodes from {ProjPath}/build/contracts files and return bytecodes dict name-bytecodes
	hashContracts := backend.readHashContracts(deployedBytecodes) // select the contracts having swarm hash and return the contracts dict hash-name

	var fuzzLoggers []*FuzzLogger
	transactions, receipients := ReadTransactions(fmt.Sprintf("%v/transactions.json", backend.ProjPath), backend.Accounts) // read transactions.json and return the types.transaction and recipent_address
	for i, transaction := range transactions {
		...
		logger := &FuzzLogger{
			Tx:                   tx,
			Logs:                 structLogger.StructLogs(), // evm logger
			ContractReceiveEther: false,
		}
		fuzzLoggers = append(fuzzLoggers, logger)
	}

	// initialize the account in evm, but why do this here
...
// contract info only from ProjPath
...
// read bytecodes from files and return the map of filename to bytecode
func (backend *Backend) readDeployedBytecodes() map[string]string {
	...
	for _, f := range files {
		...
		DeployedBytecodes[getFilename(f.Name())] = contractJSON["deployedBytecode"][2:]  // remove 0x
	}
...
func (backend *Backend) CommitTransaction(
	...
	// apply transaction on the backend and change backend, transfer toekn or create contract
	...
	// add contract info into backend, not add in evm
